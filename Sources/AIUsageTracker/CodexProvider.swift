...
struct CodexProvider {
    ...
    static func parseLive(_ rateLimit: [String: Any]) -> [LimitWindow] {
        ...
        if let w = window("primary_window", label: "Session (5h)") { windows.append(w) }
        if let w = window("secondary_window", label: "Weekly") { windows.append(w) }
        return windows
    }
    ...
    func fetchFromSessionLogs() -> ProviderUsage? {
        ...
        for line in contents.split(separator: "\n").reversed() {
            ...
            var usage = ProviderUsage(name: "Codex")
            if let w = window("primary", label: "Session (5h)") { usage.windows.append(w) }
            if let w = window("secondary", label: "Weekly") { usage.windows.append(w) }
